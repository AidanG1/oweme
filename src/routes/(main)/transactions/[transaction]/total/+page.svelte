<script lang="ts">
	...
	data.forEach((e) => {
		...
		if (food_map.has(e.email)) {
			food_map.set(e.email, [...food_map.get(e.mail), e.item])
		} else {
	...
	data.forEach((e) => {
		if (total_map.has(e.email)) {
			total_map.set(
				e.email,
				total_map.get(e.email) +
					total_map.get(e.email) +
					((e.item.amount_basis_points / 10000) * e.item.amount_cents) / 100
			)
		} else {
			total_map.set(
				e.email,
	...
	{#each food_map.keys() as email}
		<Card.Root>
			<Card.Header class="flex flex-row justify-between">
				...
				<Card.Title class="my-auto">40.00</Card.Title>
			</Card.Header>
			<Card.Content class="">
